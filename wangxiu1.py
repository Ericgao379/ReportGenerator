...
for index, url in enumerate(lines):
    ...
    # 替换非法字符的版本
    safe_name = sanitize_filename(unformatted[index])
    pictures_dir = os.path.join("pictures", safe_name)
    ...
    try:
        ...
        print(f"[{unformatted[index]}] 未找到链接：product")
    ...
    try:
        ...
        print(f"[{unformatted[index]}] 未找到链接：TERM")

    # 其他页面截图（RETURN, CONTACT, 等）
    for text in links:
        try:
            browser.find_element(By.PARTIAL_LINK_TEXT, text).click()
            filename = os.path.join(pictures_dir, f"{unformatted[index]}_{text}.png")
            browser.execute_script("document.body.style.zoom='50%'")
            #time.sleep(1)
            browser.get_screenshot_as_file(filename)
            browser.back()
